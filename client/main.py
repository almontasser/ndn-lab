...
def parse_hydra_cmd_opts() -> Namespace:
    ...
    def interpret_help() -> None:
        ...
        if set:
            if (len(sys.argv)-1 < 2):
                print("usage: ndn-client [-h] [-v] {insert,delete,fetch,query} ...")
                print("    ndn-client: a client made specifically for hydra, the NDN distributed repo.")
                print("    ('python3 ./examples/client.py' instead of 'ndn-client' if from source.)")
                print("")
                ...
                print("     usage: ndn-client insert -r REPO -f FILENAME -p PATH [-c COPIES]")
                print("     required args:")
                ...
                print("     usage: ndn-client delete -r REPO -f FILENAME")
                print("     required args:")
                print("        -r, --repoprefix REPO     |   a proper name of the repo prefix.")
                print("        -f, --filename FILENAME   |   a proper name for selected file.")
                print("")
                print("* function 'fetch':")
                print("     usage: ndn-client fetch -r REPO -f FILENAME [-p PATH]")
                print("     required args:")
                print("        -r, --repoprefix REPO     |   a proper name of the repo prefix.")
                ...
                print("     usage: ndn-client query -r REPO -q QUERY [-s SESSIONID]")
                print("     required args:")
                print("        -r, --repoprefix REPO     |   a proper name of the repo prefix.")
            ...
            sys.exit(0)
    # Command Line Parser
    parser = ArgumentParser(prog="ndn-client",add_help=False,allow_abbrev=False)
...
class Client:
    def __init__(self, app: NDNApp, client_prefix: FormalName, repo_prefix: FormalName) -> None:
        self.cinsert = InsertClient(app, client_prefix, repo_prefix)
        self.cdelete = DeleteClient(app, client_prefix, repo_prefix)
        self.cfetch = FetchClient(app, client_prefix, repo_prefix)
        self.cquery = QueryClient(app, client_prefix, repo_prefix)
...
async def run_client(app: NDNApp, args: Namespace) -> None:
  ...
  filename = None
  client = Client(app, client_prefix, repo_prefix)
